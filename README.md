...
**Author:** Bhavik  
**Course:** MCDA 5550 (Mobile Development & API Design)  
...
4. [Modern Tooling](#modern-tooling)
...
The **Hotel Reservation System** is a RESTful API built with Django and Django REST Framework for managing hotel reservations. This project demonstrates enterprise-grade architecture principles including modular design, database normalization, and containerized deployment.

### Key Features

- ✅ Modular app-based architecture for scalability
- ✅ Normalized database design with Person and Address models
- ✅ RESTful API endpoints for reservations and guest management
- ✅ UUID-based confirmation numbers for security
...
### Separation of Concerns: Three-App Structure

The project is organized into three independent, reusable Django apps that each handle a specific domain:

#### 1. **addresses** - Address Management
Handles all address-related data across the system.

```
apps/addresses/
├── models.py          # Address model (street, city, province, postal_code, country)
├── serializers.py     # AddressSerializer for API responses
├── views.py           # Address CRUD operations (if needed)
└── migrations/        # Database schema changes
```

**Why this separation?**
- Addresses are a cross-cutting concern used by Hotels and Persons
- Enables reuse across multiple apps without code duplication
- Simplifies testing and maintenance of address logic
- Follows DRY (Don't Repeat Yourself) principle

#### 2. **hotels** - Hotel Information
Manages hotel data with direct reference to addresses.

```
apps/hotels/
├── models.py          # Hotel model (name, description, contact, base_rate, address FK)
├── serializers.py     # HotelSerializer with nested AddressSerializer
├── views.py           # Hotel listing and detail views
└── migrations/        # Schema migrations
```

**Key Design Decisions:**
- Hotel has a **OneToOneField** to Address (each hotel has exactly one registered address)
- CASCADE on_delete ensures data consistency
- `related_name='hotel'` allows reverse lookups: `address.hotel`

#### 3. **reservations** - Reservation & Guest Management
Handles guest profiles and reservation bookings.

```
apps/reservations/
├── models.py          # Person and Reservation models with ManyToMany relationship
├── serializers.py     # PersonSerializer and ReservationSerializer with nested guests
├── views.py           # API views for creating and retrieving reservations
├── urls.py            # URL routing for reservation endpoints
└── migrations/        # Schema migrations
```

**Relationship Design:**
- **Person** model stores guest information with optional address reference
- **Reservation** model uses ManyToManyField to link multiple Persons as guests
- Enables flexible guest management (add/remove guests from reservations)
- Person records are reusable across multiple reservations

### Benefits of This Architecture

| Principle | Benefit |
|-----------|---------|
| **Modularity** | Each app is self-contained and independently testable |
| **Reusability** | Address logic used across Hotels and Persons without duplication |
| **Maintainability** | Changes to address logic only need to be made in one place |
| **Scalability** | New features can be added as new apps (e.g., `payments`, `notifications`) |
| **Team Collaboration** | Different developers can work on different apps simultaneously |
| **Database Performance** | Normalized schema reduces data redundancy and storage overhead |
...
└─────────────┘
      ...
      │
┌─────────────┐         ┌──────────────────┐
│   Person    │◄───M:M──│  Reservation     │
├─────────────┤         ├──────────────────┤
│ id (PK)     │         │ id (PK)          │
│ name        │         │ hotel_name       │
│ email*      │         │ checkin (DATE)   │
│ phone       │         │ checkout (DATE)  │
│ address_id* │         │ confirmation_num*│
└─────────────┘         │ guests (M2M)     │
                        └──────────────────┘
```
...
**Problem:** If we stored guest info directly in Reservation:
```json
{
  "guest_name": "John Doe",
...
**Issue:** Data redundancy - If John makes another reservation, all his info is duplicated in the database.

**Solution:** Separate Person model with ManyToMany relationship:
- ✅ Person data stored once
- ✅ Reservation references Person by ID
- ✅ One Person can have multiple Reservations
- ✅ Reduced storage, faster queries, easier updates
...
| `base_rate` | DECIMAL(10,2) | Per night rate |
| `address_id` | INT | Foreign Key → Address (OneToOne, CASCADE) |
...
| `address_id` | INT | Foreign Key → Address (Nullable, SET_NULL) |

#### Reservation Table
| Column | Type | Notes |
|--------|------|-------|
| `id` | INT | Primary Key |
| `hotel_name` | VARCHAR(255) | Hotel identifier |
| `checkin` | DATE | Check-in date |
| `checkout` | DATE | Check-out date |
| `confirmation_number` | VARCHAR(100) | Unique identifier (UUID-based) |

#### Reservation_Guests (M2M Junction Table)
| Column | Type | Notes |
|--------|------|-------|
| `id` | INT | Primary Key |
| `reservation_id` | INT | Foreign Key → Reservation |
